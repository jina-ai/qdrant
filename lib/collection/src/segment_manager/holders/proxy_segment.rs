...
impl SegmentEntry for ProxySegment {
    ...
    fn set_full_payload_with_value(&mut self, op_num: SeqNumberType, point_id: PointIdType, full_payload: TheMap<PayloadKeyType, serde_json::value::Value>) -> OperationResult<bool> {
        if self.version() > op_num { return Ok(false); }
        self.move_if_exists(op_num, point_id)?;
